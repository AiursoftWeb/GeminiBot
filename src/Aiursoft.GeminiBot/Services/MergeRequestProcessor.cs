    ...
    private async Task CheckAndFixMergeRequestAsync(MRToProcess item, Server server)
    {
        ...
        try
        {
            ...
            await workflowEngine.ExecuteAsync(context, async ctx =>
            {
                if (isOthersMr)
                {
                    await HandleOthersMrFinalizeAsync(ctx, mr, item.TargetBranch, ctx.GeminiOutput);
                }
                else
                {
...
Please analyze the feedback and make the necessary changes.";
    ...
    private async Task CreateNewMergeRequestAsync(Server server, Repository targetRepository, MergeRequestSearchResult oldMr, string targetBranch, string botBranchName, string geminiOutput)
    {
...
## Changes
This merge request contains automated fixes generated by the Gemini Bot.
